...
HRESULT D3D_Create(HWND hwnd)
{
	...
	// 頂点シェーダーオブジェクトを生成、同時に頂点レイアウトも生成
	hr = CreateVertexShader(g_pDevice, "CODE/04_hlsl/VertexShader.hlsl", "vs_main", "vs_5_0",
	...
	// ピクセルシェーダーオブジェクトを生成
	hr = CreatePixelShader(g_pDevice, "CODE/04_hlsl/PixelShader.hlsl", "ps_main", "ps_5_0", &g_pPixelShader);
	if (FAILED(hr)) {
